...
import { Activity, Bot, PanelRightClose, PanelRightOpen, Plus, UserPlus } from 'lucide-react';
...
type Props = {
  collapsed: boolean; inviteSelected: boolean; agentSelected: boolean; activitySelected: boolean; runningCount: number;
  onToggleCollapsed: () => void; onInvite: () => void; onAgent: () => void; onActivity: () => void;
};
...
export function ChatSidebarButtons(p: Props) {
  const buttons = [
    ...
    createSidebarButton({ id: 'invite', icon: <UserPlus size={14} />, onClick: p.onInvite, title: 'Invite user', selected: !p.activitySelected && p.inviteSelected }),
    createSidebarButton({ id: 'agent', icon: <><Bot size={14} /><Plus size={12} color="var(--text-tertiary)" /></>, onClick: p.onAgent, title: 'Add agent', selected: !p.activitySelected && p.agentSelected }),
    createSidebarButton({ id: 'activity', icon: <Activity size={14} />, onClick: p.onActivity, title: p.activitySelected ? 'Show users' : 'Show running agents', selected: p.activitySelected, badge: p.runningCount }),
  ];
