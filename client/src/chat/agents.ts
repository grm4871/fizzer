...
export function formatAgentChatPrompt(
  ...
  const coordinatorGuidance = registration.orchestrator
    ? ' You are this channel’s coordinator. Handle tiny Q&A and one-liner fixes directly with no mission hop. When the user proposes an actionable change or says something should work differently, treat that as authorization to implement it within the stated scope unless they clearly ask only to discuss or explain. Do not stop at agreement, praise, or a proposal: make the change, verify it, and report the result.'
      + ' Prefer creating missions for multi-step work. A mission is durable searchable task data in the chat transcript — use `cascade-chat mission start` after scope is clear, or let accept-on-clarification open the mission for you.'
      + ' For non-trivial or ambiguous work: do NOT mission-start immediately. Post a short body plus `clarification` questionnaire (in-chat radios/checkboxes/text like Claude Code / Codex): `{ title, tokenBudget?, questions: [{ id, prompt, kind: "text"|"single"|"multi", options?: string[] }] }`. Prefer `single`/`multi` with options when choices are discrete. User answers in the card and clicks Accept → mission → that freezes the kanban contract, opens a mission, and wakes you to plan/delegate. Drive until mission finish, token budget, or manual stop. Tiny Q&A skips clarification.'
  ...
  const coordinatorContinuationGuidance = registration.orchestrator
    ? ' Continue coordinating: tiny Q&A direct; non-trivial work = clarification card first (scope Qs), then Accept opens mission+contract and you drive. Delegate when another session adds value. Keep replies short; close missions after integration. Ship only after `npm run build` + green Deploy; open chat images via `cascade-chat attachment` (never “cannot see”).'
    : '';
