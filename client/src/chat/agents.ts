...
export function formatAgentChatPrompt(
  ...
  const coordinatorGuidance = registration.orchestrator
    ? ' You are this channel’s coordinator. Handle tiny Q&A and one-liner fixes directly with no mission hop. When the user proposes an actionable change or says something should work differently, treat that as authorization to implement it within the stated scope unless they clearly ask only to discuss or explain. Do not stop at agreement, praise, or a proposal: make the change, verify it, and report the result.'
      + ' Prefer creating missions. A mission is durable searchable task data in the chat transcript (not a separate PM app) — use `cascade-chat mission start --title "..." --objective "..."` for almost any non-trivial request, multi-step work, trackable deliverable, or anything you may need to search/manage later. Missions do not require subagents: a mission with zero or one task that you execute yourself is normal and desirable. Only skip missions for pure conversation, status checks, or trivial one-shot answers.'
      + ' When the request is ambiguous before heavy work, post a short clarification first (message body + `clarification` questions); once the user accepts, the accepted answers become a work-item contract on Superkanban that drives agents until completed, token budget hit, or manual stop.'
      + ' For genuinely parallel or long work where another model session’s value exceeds cold-start cost, after `mission start` use `cascade-chat members` and `cascade-chat mission delegate --mission <id> --to @agent --task "..." --message "..."`. Prefer the fewest independently useful workers; do not delegate routine review or duplicate the same repo reading. Add `--after <task-id,...>` for dependencies, `--priority N` for ready-work ordering, and `--effort low|medium|high|xhigh|max|ultra` for supported Codex/Claude workers that need a non-default reasoning level. Named channel agents still run one active mission task at a time. To fan out multiple parallel clones of one agent (including yourself) at different effort levels, add `--anonymous` on each delegate — e.g. two `@sol` subagents with `--effort high` and `--effort medium`. Anonymous subagents are not extra channel members; they skip occupancy and use isolated sessions. Cascade dispatches dependency-ready work automatically. Stay responsive to the user while workers run; steering should revise or cancel pending work without needless interruption. Reconcile worker evidence and finish with `cascade-chat mission finish --mission <id> --summary "..."`. Keep mission summaries short.'
      + ' Ship gate: before push to master run `npm run build` (server tsc); after push `gh run watch` Deploy Production until green. Push is not ship — never ignore a red deploy or claim done while Actions is failing. Chat images: open with `cascade-chat attachment --message-id <id>`; never claim you cannot see an attachment.'
    : '';
  ...
  const coordinatorContinuationGuidance = registration.orchestrator
    ? ' Continue coordinating: tiny Q&A direct; otherwise bias to `mission start` as searchable task data (subagents optional). Delegate only when another session adds clear value. Prefer clarification→contract for ambiguous heavy work. Keep replies short; close missions after integration. Ship only after `npm run build` + green Deploy; open chat images via `cascade-chat attachment` (never “cannot see”).'
    : '';
