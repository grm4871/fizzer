...
export function formatAgentChatPrompt(
  ...
  const coordinatorGuidance = registration.orchestrator
    ? ' You are this channel’s coordinator. Handle tiny Q&A and one-liner fixes directly with no mission hop. When the user proposes an actionable change or says something should work differently, treat that as authorization to implement it within the stated scope unless they clearly ask only to discuss or explain. Do not stop at agreement, praise, or a proposal: make the change, verify it, and report the result.'
      ...
      + ' For non-trivial or ambiguous work: do NOT mission-start immediately. Post a **tiny** clarification card (max 3 questions, preferably 1–2). Always **prefill** each question with `answer` set to your recommended default so Accept is one click. Prefer `single`/`multi` with short options over free text. Shape: `{ title, tokenBudget?, questions: [{ id, prompt, kind: "text"|"single"|"multi", options?, answer }] }`. User tweaks only disagreements, then Accept → mission freezes the contract, opens a mission, and wakes you to plan/delegate. Tiny Q&A skips clarification.'
  ...
  const coordinatorContinuationGuidance = registration.orchestrator
    ? ' Continue coordinating: tiny Q&A direct; non-trivial work = small prefilled clarification (≤3 Qs, defaults set), then Accept opens mission+contract. Delegate when another session adds value. Keep replies short; close missions after integration. Ship only after `npm run build` + green Deploy; open chat images via `cascade-chat attachment` (never “cannot see”).'
    : '';
