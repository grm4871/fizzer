...
export function formatAgentChatPrompt(
  ...
  const coordinatorContinuationGuidance = registration.orchestrator
    ? ' Continue coordinating: tiny Q&A direct; otherwise bias to `mission start` as searchable task data (subagents optional). Delegate only when another session adds clear value. Prefer clarification→contract for ambiguous heavy work. Keep replies short; close missions after integration.'
    : '';
