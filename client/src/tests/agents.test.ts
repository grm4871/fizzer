...
describe('formatAgentChatPrompt', () => {
  ...
  it('gives a coordinator an explicit zero-hop/direct-or-mission contract', () => {
    ...
    expect(prompt).toContain('one anonymous self-subagent');
    expect(prompt).toContain('not vault agents');
    expect(prompt).toContain('pass the request unchanged');
    expect(prompt).toContain('channel working directory');
    expect(prompt).toContain('do not plan, verify, poll, or wait');
