 ...
 */
export async function runDirectivesInNote(db: Db, noteId: string): Promise<string> {
  const note = getNote(db, noteId);
  if (!note) throw new Error('Note not found');

  const vault = db.prepare('SELECT * FROM vaults WHERE id = ?').get(note.vault_id) as any;
