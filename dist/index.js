...
import { addTag, createFolder, createNote, createVault, deleteFolder, deleteNote, ensureVaultSchema, getBacklinks, getGraph, getNote, getVault, listFolders, listNotes, listTags, listVaults, moveNote, removeTag, searchNotes, toggleArchive, togglePin, updateFolder, updateNote, } from './server/vault.js';
import { createNoteVersion, diffNoteVersions, diffText, ensureVersionsSchema, listNoteVersions, } from './server/versions.js';
import { ensureRunnerSchema, setRunEventSink, listRuns, getRun, listRunEvents, startRun, sendRunMessage, } from './server/runner.js';
...
app.post('/api/notes/:id/run-directives', requireAuth, async (req, res) => {
    const existing = getNote(db, req.params.id);
    if (!existing)
        return res.status(404).json({ error: 'Note not found' });
    const vault = getVault(db, existing.vault_id, req.user.id);
