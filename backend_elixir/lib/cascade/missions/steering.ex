defmodule Cascade.Missions.Steering do
        ...
        case snapshot(task) do
          ...
          when status in ~w(pending running) and mission_status not in ~w(completed canceled) ->
            ...
            if run && run.status in ~w(completed failed) do
              ...
              SQL.exec(
                "UPDATE chat_mission_tasks SET status='pending',prompt=?,run_id=NULL,dispatch_id=NULL,attempt=attempt+1,updated_at=datetime('now') WHERE id=?",
                [prompt, task]
              )
  ...
  def acknowledgment(id) do
    case SQL.one("SELECT e.task_id,e.attempt,r.kind,r.summary FROM chat_mission_events e LEFT JOIN chat_mission_events r ON r.source_key='steering-result:' || e.id WHERE e.id=?", [id]) do
      [task, attempt, "steering_queued", _] ->
        case SQL.one("SELECT r.id,r.status,r.owner_user_id FROM chat_mission_events e JOIN runs r ON r.id=e.run_id WHERE e.task_id=? AND e.attempt=? AND e.kind='task_started' ORDER BY e.id LIMIT 1", [task, attempt + 1]) do
          [run, status, owner] when not is_nil(owner) ->
            %{id: id, taskId: task, runId: run, status: "dispatched", detail: "Instructions assigned to worker run #{run} (#{status}); execution is not confirmed by dispatch alone"}
          _ -> queued(id, "Instructions saved; waiting for worker dispatch")
        end
